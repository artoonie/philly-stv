...
Run `npm run screenshot` and look at `screenshots/*.png`. Judge every state against two readers. The landing view is deliberately minimal (Today vs 7 × 3 STV, ring cards only, sliders hidden); `default-*.png` is what a first-time visitor sees, `party-all-charts.png` is everything switched on.
